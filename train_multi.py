...
df_left = df[df['laterality'] == 'L'].reset_index() 

df_cc_left = df_left[df_left['view'] == 'CC'].reset_index()
df_mlo_left = df_left[df_left['view'] == 'MLO'].reset_index()
print(df_cc_left.iloc[0])
...
class CancerDataset(Dataset):
    def __init__(self, df, df_mlo, root_dir, transform=None):
        """
        Args:
    ...
    def __getitem__(self, index):
        ...
        patient_id = self.patient_data['patient_id'].iloc[index]
        mlo = self.patient_mlo[self.patient_mlo['patient_id'] == patient_id].iloc[0]
        mlo_name = str(mlo['patient_id'] + '_' + str(mlo['image_id'])) + '.png'
        print(mlo_name)
...
train_cancer_dataset = CancerDataset(df_cc_left_train, df_mlo_left, data_dir, transform)
test_cancer_dataset = CancerDataset(df_cc_left_test, df_mlo_left, data_dir, test_transform)
